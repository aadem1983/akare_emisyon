...
def load_pandas():
    ...
    if not pandas_loaded or pd is None:
        import importlib
...
def export_baca_bilgileri_excel():
    try:
        ...
        # Güvenli: pandas modülünü lazy loader ile al (fallback ile)
        try:
            pd = load_pandas()
            if pd is None:
...
def save_parametre_fields_table():
    """'İçe/Dışa Aktarma Format Tablosu' gövdesindeki hücreleri alıp kaydeder.

    İstemci, tbody satırlarını rows: [[c1,...,c9], ...] olarak gönderir.
    İlk 7 sütun sırasıyla: genel, nem, yg, toc, voc, pm10, ct.
    Son iki 'boş' sütun yok sayılır.
    """
    try:
        ...
        def add_if_value(target_list, value):
            if value is None:
                return
            value_str = str(value).strip()
            if value_str and value_str not in target_list:
                target_list.append(value_str)

        for row in rows:
            # Güvenli erişim ve sütun sayısını kontrol et
            if not isinstance(row, list):
                continue
            # En az 7 sütun beklenir (genel..ct); fazlası yok sayılır
            cols = row + ["", "", "", "", "", "", ""]  # pad
            add_if_value(new_fields['genel'], cols[0])
            add_if_value(new_fields['nem'], cols[1])
            add_if_value(new_fields['yg'], cols[2])
            add_if_value(new_fields['toc'], cols[3])
            add_if_value(new_fields['voc'], cols[4])
            add_if_value(new_fields['pm10'], cols[5])
            add_if_value(new_fields['ct'], cols[6])
        ...
        total = sum(len(v) for v in new_fields.values())
        return jsonify({'success': True, 'message': f'{total} alan kaydedildi.'})
    except Exception as e:
        print(f"Parametre alanları tablo kaydı hatası: {e}")
...
def create_baca_word_document_from_template(baca_bilgisi, baca_parametreleri, form_bilgisi):
    ...
    try:
        ...
        # Firma adını ilk 3 kelime ile sınırla
        ...
        data = {
            'FIRMA_ADI': first_three_words(baca_bilgisi.get('firma_adi', '')).title(),  # İlk 3 kelime
            'OLCUM_KODU': baca_bilgisi.get('olcum_kodu', ''),
        ...
        # Şablondaki bilgi tablosunda (Firma Adı / Ölçüm Kodu / Baca Adı / Tarih) genişlik ayarı
        try:
            for table in doc.tables:
                # Tablo içinde "Firma Adı" metni geçen bir hücre var mı?
                has_firma = any('Firma Adı' in (cell.text or '') for row in table.rows for cell in row.cells)
                if not has_firma:
                    continue
                # 4 sütunlu bilgi tablosu varsayımı ile kolon genişliklerini ayarla
                if len(table.columns) >= 4:
                    table.autofit = False
                    table.allow_autofit = False
                    # Etiket kolonları daralt, değer kolonlarını genişlet
                    try:
                        table.columns[0].width = Inches(1.1)  # Etiket: "Firma Adı"
                        table.columns[1].width = Inches(3.0)  # Değer: firma adı (genişletildi)
                        table.columns[2].width = Inches(1.1)  # Etiket: "Ölçüm Kodu" vb.
                        table.columns[3].width = Inches(2.0)  # Değer
                    except Exception:
                        pass
                # İlk eşleşmeden sonra çık
                break
        ...
        try:
            ...
            tarih_footer = datetime.now().strftime('%d.%m.%Y')
            footer_text = f"BU FORM {tarih_footer} TARİHİNDE {personel_adi} TARAFINDAN OLUŞTURULMUŞTUR."

            # Footer nesnesi
            if doc.sections:
                footer = doc.sections[0].footer
                # Sol tarafta bilgi cümlesi
                left_p = footer.add_paragraph(footer_text)
                ...
                # Sağ tarafta tarih, ad-soyad ve imza
                right_p = footer.add_paragraph()
                right_p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
                r1 = right_p.add_run(tarih_footer)
                right_p.add_run().add_break()
                ...
                name_clean = normalize_name(personel_adi).strip().replace(' ', '_')
                candidates = [
                    os.path.join(os.path.dirname(__file__), 'static', 'images', 'signatures', f'{name_clean}.png'),
                    os.path.join(os.path.dirname(__file__), 'static', 'images', 'signatures', f'{name_clean}.jpg'),
                    os.path.join(os.path.dirname(__file__), 'static', 'images', f'imza_{name_clean.lower()}.png'),
                    os.path.join(os.path.dirname(__file__), 'static', 'images', f'imza_{name_clean.lower()}.jpg'),
                ]
